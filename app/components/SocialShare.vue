...
const shareTwitter = (): void => {
  const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(props.articleTitle)}&url=${encodeURIComponent(props.articleUrl)}`
  window.open(url, '_blank', 'width=600,height=400')
}

const shareLinkedIn = (): void => {
  const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(props.articleUrl)}`
  window.open(url, '_blank', 'width=600,height=400')
}

const shareFacebook = (): void => {
  const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(props.articleUrl)}`
  window.open(url, '_blank', 'width=600,height=400')
}
...
<template>
  <div id="social-share" class="bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-lg p-5">
    ...
    <p v-if="copied" class="text-success text-xs mt-2">Link copiado!</p>
  </div>
</template>
